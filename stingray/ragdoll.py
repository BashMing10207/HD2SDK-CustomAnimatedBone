...
class StingrayRagdoll:
    ...
    def Serialize(self, f: MemoryStream):
        if f.IsReading():
            ...
            try:
                ...
                PrettyPrint(f"HavokLib unpack for ragdoll failed: {e}", "error")
        else: # Writing
            try:
                import lxml.etree as etree
            except ImportError:
            ...
            if self.is_modified and self.IsXml:
                try:
                    ...
                    print(f"HavokLib pack for ragdoll failed: {e}")
                    f.write(self.Raw)
            else:
                f.write(self.Raw)
        return self
    ...
    def sync_with_skeleton(self, skeleton: VirtualSkeleton):
        """
        .ragdoll 파일의 본 데이터를 가상 스켈레톤(SoT)에 맞춰 동기화합니다.
        """
        try:
            import lxml.etree as etree
        except ImportError:
        ...
        if not self.IsXml:
            PrettyPrint(f"Cannot sync ragdoll: not valid XML.", "warn")
            return False

        PrettyPrint(f"Syncing ragdoll with virtual skeleton...", "info")
        root = etree.fromstring(self.Xml)

        # Ragdoll 파일 내의 모든 hkaSkeleton 객체를 찾습니다.
        skeleton_objs = root.findall(".//hkobject[@class='hkaSkeleton']")
        if not skeleton_objs:
            PrettyPrint(f"Could not find any hkaSkeleton object in ragdoll. Skipping.", "warn")
            return False
        ...
        for i, skeleton_obj in enumerate(skeleton_objs):
            PrettyPrint(f"  - Patching skeleton #{i+1}/{len(skeleton_objs)} in ragdoll...", "info")
        ...
        PrettyPrint(f"Successfully patched {len(skeleton_objs)} skeleton(s) in ragdoll.", "success")
        return True